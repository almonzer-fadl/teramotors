...
class PDFGenerator {
  ...
  async generateInvoicePDF(invoice, jobCard, options = { language: 'ar' }) {
  ...
  generateInvoiceHTML(invoice, jobCard, options) {
    ...
    const servicesTotal = services.reduce((sum, s) => sum + (s.laborHours * s.laborRate), 0);
...
<body>
  ...
  getStatusText(status, isRTL) {
    ...
    return statusMap[isRTL ? 'ar' : 'en'][status] || status;
  }
}
