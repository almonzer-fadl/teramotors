...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Generate invoice number once at the start

    const generatedInvoiceNumber = await Invoice.getNextInvoiceNumber((session.user as any).tenantId);
    ...
    if (jobCardId) {
      ...
      const zatcaResult = await invoiceService.createInvoiceFromJobCard(generatedInvoiceNumber, {
      ...
      if (zatcaResult.success && zatcaResult.invoice) {
        ...
        // Store ZATCA data, using the generatedInvoiceNumber as the source of truth
        ...
        zatcaData = {
          ...
          invoiceNumber: generatedInvoiceNumber, // Use the generated invoice number
      ...
      try {
        ...
        const zatcaResult = await invoiceService.createInvoice(generatedInvoiceNumber, {
    ...
    if (jobCardId && autoCloseJobCard) {
      ...
      try {
        const customerIdString =
          typeof customerId === 'object' && customerId._id
            ? customerId._id.toString()
            : customerId.toString();
        const jobCardIdString =
          typeof jobCardId === 'object' ? jobCardId?.toString() : jobCardId;
        if (!jobCardIdString) {
          throw new Error('Missing job card ID for WhatsApp notification');
        }
