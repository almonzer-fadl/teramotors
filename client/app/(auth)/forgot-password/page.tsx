  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md bg-white shadow rounded-lg p-6">
        <h1 className="text-xl font-semibold text-gray-900 mb-4">{t('auth.forgot_your_password')}</h1>
        <p className="text-sm text-gray-500 mb-6">{t('auth.forgot_password_prompt')}</p>
        {sent ? (
        ...
        <div className="mt-6 text-sm text-gray-500">
          <Link href="/login" className="text-blue-600 hover:text-blue-700">{t('auth.back_to_login')}</Link>
        </div>
      </div>
    </div>
  )
