  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md bg-white shadow rounded-lg p-6">
        <h1 className="text-xl font-semibold text-gray-900 mb-4">{t('auth.reset_password')}</h1>
        {done ? (
          <div className="space-y-4">
            <div className="text-sm text-green-700 bg-green-50 border border-green-200 rounded p-3">{t('auth.password_reset_success')}</div>
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <button disabled={submitting || password !== confirm} className="w-full inline-flex justify-center rounded-md bg-blue-600 px-4 py-2 text-white text-sm font-medium hover:bg-blue-700 disabled:opacity-50">
              {submitting ? t('auth.saving') : t('auth.reset_password')}
            </button>
          </form>
        )}
      </div>
