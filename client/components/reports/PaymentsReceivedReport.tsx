        ...
        return (
             <div className="text-center py-20 bg-gray-100/50 dark:bg-gray-800/20 rounded-2xl">
                ...
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    {t('reports.payments.no_payments', 'There are no completed payments within the selected date range.')}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <StatCard
                    title={t('reports.payments.total_received', 'Total Payments Received')}
                    value={formatCurrency(totalPaymentsReceived)}
                    icon={<DollarSign className="w-7 h-7 text-white" />}
                    color="bg-gradient-to-br from-green-500 to-emerald-600"
                    tooltip={t('reports.payments.total_received_tooltip', 'Sum of all completed payments in the period.')}
                />
                <StatCard
                    title={t('reports.payments.total_transactions', 'Total Transactions')}
                    value={paymentCount.toString()}
                    icon={<FileText className="w-7 h-7 text-white" />}
                    color="bg-gradient-to-br from-blue-500 to-indigo-600"
                    tooltip={t('reports.payments.total_transactions_tooltip', 'Total number of completed payment transactions.')}
                />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                 <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm p-6 rounded-3xl shadow-lg shadow-gray-200/50 dark:shadow-black/30 border border-gray-100 dark:border-gray-800">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">{t('reports.payments.by_method', 'Payments by Method')}</h3>
                ...
                <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm rounded-3xl shadow-lg shadow-gray-200/50 dark:shadow-black/30 border border-gray-100 dark:border-gray-800 overflow-hidden">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white p-6">{t('reports.payments.detailed_list', 'Detailed Payments List')}</h3>
                    <div className="overflow-y-auto h-[300px]">
                         <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead className="bg-gray-50/80 dark:bg-gray-800/80 sticky top-0">
                                <tr>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t('reports.payments.date', 'Date')}</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t('reports.payments.invoice_number', 'Invoice #')}</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t('reports.payments.amount', 'Amount')}</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t('reports.payments.method', 'Method')}</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
                                {payments.map((payment: any) => (
                                    <tr key={payment._id}>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{new Date(payment.paymentDate).toLocaleDateString(i18n.language)}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">{payment.invoiceId?.invoiceNumber || t('reports.payments.not_available', 'N/A')}</td>
